...
func _ready() -> void:
    CardEvents.card_aim_started.connect(_on_card_aim_started)
    CardEvents.card_aim_ended.connect(_on_card_aim_ended)
...
func _get_points() -> Array:
    ...
    start.y += (current_card.size.y/2)
    start.x += (current_card.size.x)
...
func _on_card_aim_started(card: CardUI) -> void:
    if not card.card.is_state_card():
        return
